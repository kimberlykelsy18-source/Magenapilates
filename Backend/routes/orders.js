const express = require('express');
const flutterwave = require('../services/flutterwave');

// PRE-A001, PRE-B001, ...
function toShortOrderId(n) {
  if (!n) return 'PRE-???';
  const letterIndex = Math.floor((n - 1) / 999);
  const numPart = ((n - 1) % 999) + 1;
  const letter = String.fromCharCode(65 + letterIndex);
  return `PRE-${letter}${String(numPart).padStart(3, '0')}`;
}

module.exports = ({ supabase, serviceSupabase, transporter }) => {
  const router = express.Router();

  // Public — get order by ID (used on order-success page)
  router.get('/api/orders/:id', async (req, res) => {
    const { id } = req.params;

    const { data, error } = await serviceSupabase
      .from('pre_orders')
      .select('id, order_number, product_name, order_type, quantity, total_amount, deposit_amount, payment_method, status, customer_name, customer_email, created_at')
      .eq('id', id)
      .single();

    if (error || !data) return res.status(404).json({ error: 'Order not found' });
    res.json({ ...data, short_id: toShortOrderId(data.order_number) });
  });

  // Public — create pre-order and initiate Flutterwave payment
  router.post('/api/orders', async (req, res) => {
    const {
      product_id,
      product_name,
      order_type,
      quantity,
      wants_engraving,
      customer_name,
      customer_email,
      customer_phone,
      customer_address,
      notes,
      total_amount,
      deposit_amount,
      payment_method,
    } = req.body;

    if (!product_id || !product_name || !order_type || !customer_name || !customer_email || !customer_phone || !total_amount || !payment_method) {
      return res.status(400).json({ error: 'Missing required fields' });
    }

    // Insert the pre-order
    const { data: order, error: orderError } = await serviceSupabase
      .from('pre_orders')
      .insert({
        product_id,
        product_name,
        order_type,
        quantity: quantity || 1,
        wants_engraving: wants_engraving || false,
        customer_name,
        customer_email,
        customer_phone,
        customer_address,
        notes,
        total_amount,
        deposit_amount: deposit_amount || 0,
        payment_method,
        status: 'pending_payment',
      })
      .select()
      .single();

    if (orderError) {
      console.error('[Orders] Insert error:', orderError.message);
      return res.status(500).json({ error: orderError.message });
    }

    const shortId = toShortOrderId(order.order_number);

    if (payment_method === 'card') {
      try {
        // Unique tx_ref for this transaction
        const txRef = `mgn-${order.id}`;
        const amountDue = Number(total_amount) + Number(deposit_amount || 0);

        let paymentPlanId = null;

        // ── Rental: create a monthly payment plan ─────────────────────────────
        // The first payment (deposit + first month) is charged via the checkout.
        // Subsequent months are auto-billed by Flutterwave using the plan amount.
        if (order_type === 'rental') {
          const rentalAmount = Number(total_amount);   // monthly rental rate
          const fixedMonths = 5;                       // adjust from site_settings if needed
          // duration = total billing cycles (months 1 through N).
          // Month 1 is captured by the checkout; Flutterwave handles months 2–N via the plan.
          const plan = await flutterwave.createPaymentPlan({
            name: `${product_name} — Monthly Rental (${shortId})`,
            amount: rentalAmount,
            duration: fixedMonths,
            currency: 'KES',
            interval: 'monthly',
          });
          paymentPlanId = plan.id;

          // Store plan ID on the payment record (inserted below)
          console.log(`[Orders] Created payment plan ${plan.id} for order ${shortId}`);
        }

        // ── Initiate Flutterwave hosted checkout ──────────────────────────────
        const result = await flutterwave.initiatePayment({
          txRef,
          amount: amountDue,
          currency: 'KES',
          redirectUrl: `${process.env.FRONTEND_URL}/order-success`,
          customer: {
            email: customer_email,
            phone_number: customer_phone,
            name: customer_name,
          },
          customizations: {
            title: 'Magena Pilates',
            description: `${product_name} — ${order_type === 'rental' ? 'Monthly Rental' : 'Purchase'} (${shortId})`,
            logo: `${process.env.FRONTEND_URL}/logo.png`,
          },
          paymentPlanId,
        });

        // Record payment entry
        await serviceSupabase.from('payments').insert({
          order_id: order.id,
          checkout_request_id: txRef,          // tx_ref used to match on redirect
          amount: amountDue,
          payment_method: 'card',
          status: 'pending',
          flw_plan_id: paymentPlanId ? String(paymentPlanId) : null,
        });

        return res.status(201).json({
          order_id: order.id,
          short_id: shortId,
          redirect_url: result.link,
          tx_ref: txRef,
        });
      } catch (err) {
        console.error('[Orders] Flutterwave error:', err.message);
        await serviceSupabase
          .from('pre_orders')
          .update({ status: 'cancelled' })
          .eq('id', order.id);
        return res.status(502).json({ error: 'Payment gateway error. Please try again.' });
      }
    }

    // M-PESA — manual paybill flow (payment processed offline, admin confirms)
    if (payment_method === 'mpesa') {
      return res.status(201).json({
        order_id: order.id,
        short_id: shortId,
        payment_method: 'mpesa',
        mpesa_paybill: process.env.MPESA_PAYBILL || '',
        message: 'Order received. Please complete M-PESA payment using the paybill details provided.',
      });
    }

    res.status(400).json({ error: 'Unsupported payment method' });
  });

  return router;
};
