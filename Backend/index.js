...
app.get('/api/settings', async (_req, res) => {
  const { data, error } = await supabase
    .from('site_settings')
    .select('terms, engraving_price, rental_fixed_months, rental_deposit_formula, exchange_rate, instagram_url, pinterest_url, whatsapp_number, footer_disclaimer, post_order_message, waitlist_message, leather_finishes, wood_finishes')
    .eq('id', 1).maybeSingle();
